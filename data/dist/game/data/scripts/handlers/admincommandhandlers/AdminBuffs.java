...
public class AdminBuffs implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		if (command.startsWith("admin_getbuffs"))
		{
			...
			if (st.hasMoreTokens())
			{
				...
				activeChar.sendMessage("角色 " + playername + " 不在线.");
				return false;
			}
		...
		else if (command.startsWith("admin_stopbuff"))
		{
			...
			{
				activeChar.sendMessage("移除效果失败: " + e.getMessage());
				activeChar.sendMessage("用法: //stopbuff <objectId> <skillId>");
				return false;
			}
		}
		else if (command.startsWith("admin_stopallbuffs"))
		{
			...
			{
				activeChar.sendMessage("移除所有效果失败: " + e.getMessage());
				activeChar.sendMessage("用法: //stopallbuffs <objectId>");
				return false;
			}
		}
		else if (command.startsWith("admin_areacancel"))
		{
			...
			try
			{
				...
				activeChar.sendMessage("半径 " + radius + "返回内所有效果被移除");
				return true;
			}
			catch (NumberFormatException e)
			{
				activeChar.sendMessage("用法: //areacancel <radius>");
				return false;
			}
		}
		else if (command.startsWith("admin_removereuse"))
		{
			...
			activeChar.sendMessage("重复技能已从角色  " + creature.getName() + "身上移除.");
			return true;
		}
		else if (command.startsWith("admin_switch_gm_buffs"))
		{
			if (Config.GM_GIVE_SPECIAL_SKILLS != Config.GM_GIVE_SPECIAL_AURA_SKILLS)
			{
				...
				activeChar.sendMessage("你改变了目标的 " + (toAuraSkills ? "光环" : "一个") + " 特殊技能.");
				return true;
			}
			activeChar.sendMessage("已经没有什么可以切换的了.");
			return false;
		}
		return true;
	}
	...
	public static void showBuffs(L2PcInstance activeChar, L2Character target, int page, boolean passive)
	{
		...
		final StringBuilder html = StringUtil.startAppend(500 + (effects.size()
			* 200), "<html><table width=\"100%\"><tr><td width=45><button value=\"首页\" action=\"bypass -h admin_admin\" width=45 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td><td width=180><center><font color=\"LEVEL\">效果来自 ", target.getName(), "</font></td><td width=45><button value=\"返回\" action=\"bypass -h admin_current_player\" width=45 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr></table><br><table width=\"100%\"><tr><td width=200>Skill</td><td width=30>剩余时间</td><td width=70>Action</td></tr>");
		...
		// Buttons
		StringUtil.append(html, "<br><center><button value=\"刷新\" action=\"bypass -h admin_getbuffs", (passive ? "_ps " : " "), target.getName(), "\" width=80 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
		StringUtil.append(html, "<button value=\"全部移除\" action=\"bypass -h admin_stopallbuffs ", Integer.toString(target.getObjectId()), "\" width=80 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"><br>");
		// Legend
		if (!passive)
		{
	...
	private static void removeBuff(L2PcInstance activeChar, int objId, int skillId)
	{
		...
		if ((target != null) && (skillId > 0))
		{
			if (target.isAffectedBySkill(skillId))
			{
				...
				activeChar.sendMessage("从目标 " + target.getName() + " (" + objId + "). 移除技能 "  + skillId + " 的效果 " );
			}
	...
	private static void removeAllBuffs(L2PcInstance activeChar, int objId)
	{
		...
		if (target != null)
		{
			...
			activeChar.sendMessage("从目标" + target.getName() + " (" + objId + ")移除所有效果");
			showBuffs(activeChar, target, 1, false);
			if (Config.GMAUDIT)
			{

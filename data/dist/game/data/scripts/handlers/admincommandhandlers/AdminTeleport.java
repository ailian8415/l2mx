...
public class AdminTeleport implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		{
			activeChar.sendMessage("瞬间移动准备就绪，请点击你要移动到的地方.");
			activeChar.setTeleMode(1);
		}
		...
		else if (command.startsWith("admin_recall "))
		{
			try
			{
				...
				if (param.length != 2)
				{
					activeChar.sendMessage("用法: //recall <playername>");
					return false;
				}
		...
		else if (command.startsWith("admin_go"))
		{
			...
			{
				activeChar.sendMessage("用法: //go<north|south|east|west|up|down> [offset] (default 150)");
			}
		}
		else if (command.startsWith("admin_sendhome"))
		{
			...
			if (st.countTokens() > 1)
			{
				activeChar.sendMessage("用法: //sendhome <playername>");
			}
	...
	private void teleportTo(L2PcInstance activeChar, String Coords)
	{
		try
		{
			...
			activeChar.sendMessage("你已传送至 " + Coords);
		}
	...
	private void teleportCharacter(L2PcInstance player, Location loc, L2PcInstance activeChar)
	{
		if (player != null)
		{
			...
			if (player.isJailed())
			{
				activeChar.sendMessage("对不起，角色" + player.getName() + " 被监禁.");
			}
			else
			{
				...
				if ((activeChar != null) && (activeChar.getInstanceId() >= 0))
				{
					...
					activeChar.sendMessage("你召唤了玩家 " + player.getName());
				}
				else
				{
					player.setInstanceId(0);
				}
				player.sendMessage("管理员对你进行了传送.");
	...
	private void teleportToCharacter(L2PcInstance activeChar, L2Object target)
	{
		...
		else
		{
			...
			activeChar.sendMessage("你已传送到玩家 " + player.getName() + "附近.");
		}
	}
	...
	private void changeCharacterPosition(L2PcInstance activeChar, String name)
	{
			...
			PreparedStatement ps = con.prepareStatement("UPDATE characters SET x=?, y=?, z=? WHERE char_name=?"))
		{
			...
			if (count == 0)
			{
				activeChar.sendMessage("角色不在线或位置不存在.");
			}
			else
			{
				activeChar.sendMessage("将玩家 [" + name + "] 位置设定为 (" + x + "," + y + "," + z + ").");
			}
		}

				...
				switch (subCmd)
				{
						...
						switch (announce.getType())
						{
							...
							case AUTO_NORMAL:
							{
								switch (type)
								{
									...
									default:
									{
										activeChar.sendMessage("公告类型只能设定为 AUTO_NORMAL 或 AUTO_CRITICAL!");
										return false;
									}
								}
								break;
							...
							case CRITICAL:
							{
								switch (type)
								{
									...
									default:
									{
										activeChar.sendMessage("公告类型只能更改为 NORMAL 或 CRITICAL!");
										return false;
									}
								}
								break;
					...
					case "restart":
					{
						if (!st.hasMoreTokens())
						{
							...
							activeChar.sendMessage("自动公告已重启");
							break;
						}
						String token = st.nextToken();
						if (!Util.isDigit(token))
						...
						if (announce != null)
						{
							...
							{
								activeChar.sendMessage("此选项只对自动公告有效!");
							}
						}
						else
						{
					...
					case "list":
					{
						...
						}, announcement ->
						{
							...
							{
								sb.append("<td width=60><button action=\"bypass -h admin_announces restart " + announcement.getId() + "\" value=\"重启\" width=\"60\" height=\"21\" back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>");
							}
							else
							{

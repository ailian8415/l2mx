	...
	private static void onlineChange(L2PcInstance activeChar, L2PcInstance player, int lvl)
	{
		if (lvl >= 0)
		{
			if (AdminData.getInstance().hasAccessLevel(lvl))
			{
				...
				player.sendMessage("你的权限等级变更到 " + acccessLevel.getName() + " (" + acccessLevel.getLevel() + ").");
				activeChar.sendMessage(player.getName() + "的权限等级变更到 " + acccessLevel.getName() + " (" + acccessLevel.getLevel() + ").");
			}
			else
			{

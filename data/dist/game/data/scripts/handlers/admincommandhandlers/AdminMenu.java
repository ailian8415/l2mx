...
public class AdminMenu implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_recall_char_menu"))
		{
			try
			{
				...
				teleportCharacter(player, activeChar.getLocation(), activeChar, "管理员对你进行传送.");
			}
		...
		else if (command.startsWith("admin_recall_party_menu"))
		{
			try
			{
				...
				if (!player.isInParty())
				{
					activeChar.sendMessage("角色不再队伍中.");
					teleportCharacter(player, activeChar.getLocation(), activeChar, "管理员对你进行传送.");
					return true;
				}
				for (L2PcInstance pm : player.getParty().getMembers())
				{
					teleportCharacter(pm, activeChar.getLocation(), activeChar, "管理员对你的队伍进行传送.");
				}
			}
			catch (Exception e)
			{
		...
		else if (command.startsWith("admin_recall_clan_menu"))
		{
			try
			{
				...
				{
					teleportCharacter(member, activeChar.getLocation(), activeChar, "管理员对你的血盟进行传送..");
				}
			}
			catch (Exception e)
			{
		...
		else if (command.startsWith("admin_ban_menu"))
		{
			...
			if (st.countTokens() > 1)
			{
				...
				{
					activeChar.sendMessage("你无权使用此命令!");
	...
	private void handleKill(L2PcInstance activeChar, String player)
	{
		...
		if (player != null)
		{
			...
			{
				target = plyr;
				activeChar.sendMessage("你杀死了 " + plyr.getName());
			}
		}
		if (target != null)
		{
	...
	private void teleportToCharacter(L2PcInstance activeChar, L2Object target)
	{
		...
		else
		{
			...
			activeChar.sendMessage("你将自己传送到角色 " + player.getName() + "附近");
		}
		showMainPage(activeChar);
	}

...
public final class L2PcInstance extends L2Playable
{
	...
	public int giveAvailableSkills(boolean includedByFs, boolean includeAutoGet)
	{
		...
		{
			sendMessage("你学会了 " + skillCounter + " 个新技能.");
		}
		return skillCounter;
	}
			...
			try (ResultSet rs = ps.executeQuery())
			{
				while (rs.next())
				{
					...
					if (Config.SKILL_CHECK_ENABLE && (!canOverrideCond(PcCondOverride.SKILL_CONDITIONS) || Config.SKILL_CHECK_GM))
					{
						...
						{
							Util.handleIllegalPlayerAction(this, "玩家 " + getName() + " 拥有无效的技能 " + skill.getName() + " (" + skill.getId() + "/" + skill.getLevel() + "), 职业:" + ClassListData.getInstance().getClass(getClassId()).getClassName(), IllegalActionPunishmentType.BROADCAST);
